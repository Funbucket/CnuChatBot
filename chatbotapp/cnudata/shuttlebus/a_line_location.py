...
def get_departure_time_and_gap():
    ...
    end_times = []
    for i in range(15):
        end_times.append(departure_times[i] + timedelta(minutes=16, seconds=20))
    ...
    if departure_times[14] < now <= end_times[14]:
        return departure_times[14], (now - departure_times[14]).total_seconds()
    # 운행종료
...
from chatbotapp.cnudata.time import get_time
...
def get_departure_time_and_gap():
    ...
    start_time_1 = get_time(8,30,0)
    start_time_2 = get_time(9,00,0)
    start_time_3 = get_time(9,30,0)
    start_time_4 = get_time(10,00,0)
    start_time_5 = get_time(11,00,0)
    start_time_6 = get_time(11,30,0)
    start_time_7 = get_time(13,00,0)
    start_time_8 = get_time(13,30,0)
    start_time_9 = get_time(14,30,0)
    start_time_10 = get_time(15,00,0)
    start_time_11 = get_time(15,30,0)
    start_time_12 = get_time(16,00,0)
    start_time_13 = get_time(16,30,0)
    start_time_14 = get_time(17,00,0)
    start_time_15 = get_time(17,30,0)
...
# 8시30 < who  8분 < how 8시38분에 찍


def get_a1_answer():
    who = get_departure_time_and_gap()[0]
    how = get_departure_time_and_gap()[1]
    # 현재 시간 now 가 배차가 있는 시간이라면 아래 실행
    if(type(how) == float):
        if 0<how<30:
            where_now = "정심화국제문화회관에서 출발"
