...
def get_studenthall23_answer_info(name):
    ...
    #평일이라면
        ...
        if name == "제2학생회관":
            ...
            teacher_price = teacher.find_all("td", attrs={"height": "20"})[4].get_text().strip()
        ...
        response_text += "👉학생식당" + student_price + student_menu + "\n"
        response_text += "\n👉교직원식당" + teacher_price + teacher_menu + "\n"
        response_text += "\n👉학생식당(일품)" + student_special_price + student_special_menu
        # print(response_text)
        answer = insert_text(response_text)
        reply = make_reply("다른 식당 메뉴보기", "학식")
        answer = insert_replies(answer, reply)
